...
transformations = re.search(r'CROSS JOIN|INNER JOIN.*?(?=;)', query, re.DOTALL).group(0)
...
with open(file_path, 'r') as file:
 ...
 # Extract the FROM clause
    from_clause_search = re.search(r'FROM\s+(.*?)\s+(?:CROSS JOIN|INNER JOIN|WHERE|ORDER BY|LIMIT)', query, re.DOTALL)
    if from_clause_search:
        from_clause = from_clause_search.group(1)
